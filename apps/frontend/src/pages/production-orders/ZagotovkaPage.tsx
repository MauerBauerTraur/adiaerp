...
function StoreDetail({
  ...
  const checkedCount = group.orders.filter((o) => checkedIds.has(o.id)).length;
  const allDone = checkedCount === group.orders.length && group.orders.length > 0;
  const remaining = group.orders.filter((o) => !checkedIds.has(o.id));
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {!allDone && remaining.length > 0 && (
            <button
              type="button"
              onClick={() => onCheckAll(remaining.map((o) => o.id))}
...
export function ZagotovkaPage() {
  ...
  const subOrders = useMemo(
    () => (orders ?? []).filter((o) => o.parent_production_order_id != null),
    [orders],
  );
  ...
  return (
    <div className="flex flex-col gap-4 p-4 md:p-6">
        ...
        action={
          <div className="flex items-center gap-2">
            {storeGroups.length > 0 && (
              <button
                type="button"
                onClick={() => openGlobalPrint(storeGroups, today)}
      ...
      {!isLoading && !error && tab === 'berish' && (
        storeGroups.length === 0 ? (
          <EmptyState message="Bugun uchun topshiriqlar topilmadi." />
        ) : (
          <div className="flex gap-3 items-start">
            {/* Left: store list */}
            <div className="w-56 shrink-0 space-y-1">
              {storeGroups.map((group) => {
                ...
                const checked = group.orders.filter((o) => checkedIds.has(o.id)).length;
