#!/usr/bin/env python3
"""
Deploy: sales filter (store + product_type) + StockReportPage auto-preset.

O'zgargan fayllar:
  backend: apps/backend/src/routes/sales.ts   (product_type filter qo'shildi)
  frontend: SalesChart.tsx, StockReportPage.tsx (filtr UI)

Nima qiladi:
  1. sales.ts → serverga SFTP, backend build, PM2 restart
  2. Frontend mahalliy build → SFTP dist upload
"""
...
def run(cmd: str, timeout: int = 300, check: bool = True) -> int:
    chan = c.get_transport().open_session()  # type: ignore[union-attr]
    chan.get_pty()
...
# ── 1. Backend: upload sales.ts ───────────────────────────────────
print('\n=== 1. Upload backend: sales.ts ===')
upload_file(
    ROOT / 'apps/backend/src/routes/sales.ts',
    f'{APP_DIR}/apps/backend/src/routes/sales.ts',
)
...
# ── 2. Frontend: mahalliy build + SFTP upload ─────────────────────
